...
# A fixed, tiny universe: the CBOE/ICE US Treasury constant-maturity yield
# indices Yahoo publishes (quoted in percent; no volume). Not downloadable
# from anywhere - the list IS the universe.
_RATE_INDICES = [
    ("^IRX", "US 13-week Treasury bill (discount rate)"),
...
def _universe_rate_indices(spec: ExchangeSpec, args) -> pd.DataFrame:
    return pd.DataFrame(
        [{"ticker": t, "code": t, "name": n} for t, n in _RATE_INDICES]
    )


# Another fixed list: the liquid CME Group / ICE US continuous front-month
# futures Yahoo serves ("=F" tickers). Not downloadable - curated here.
_US_FUTURES = [
    # equity index
    ("ES=F", "index", "E-mini S&P 500"),
    ("NQ=F", "index", "E-mini Nasdaq 100"),
    ("YM=F", "index", "E-mini Dow"),
    ("RTY=F", "index", "E-mini Russell 2000"),
    ("NKD=F", "index", "Nikkei 225 (CME)"),
    # rates
    ("ZQ=F", "rate", "30-Day Fed Funds"),
    ("ZT=F", "rate", "2-Year T-Note"),
    ("ZF=F", "rate", "5-Year T-Note"),
    ("ZN=F", "rate", "10-Year T-Note"),
    ("TN=F", "rate", "Ultra 10-Year T-Note"),
    ("ZB=F", "rate", "30-Year T-Bond"),
    ("UB=F", "rate", "Ultra T-Bond"),
    ("SR3=F", "rate", "3-Month SOFR"),
    # energy
    ("CL=F", "energy", "WTI Crude Oil"),
    ("BZ=F", "energy", "Brent Crude Oil"),
    ("NG=F", "energy", "Natural Gas"),
    ("RB=F", "energy", "RBOB Gasoline"),
    ("HO=F", "energy", "NY Harbor ULSD"),
    # metals
    ("GC=F", "metal", "Gold"),
    ("SI=F", "metal", "Silver"),
    ("HG=F", "metal", "Copper"),
    ...
    ("ZC=F", "ag", "Corn"),
    ("ZW=F", "ag", "Chicago SRW Wheat"),
    ("KE=F", "ag", "KC HRW Wheat"),
    ("ZS=F", "ag", "Soybeans"),
    ("ZM=F", "ag", "Soybean Meal"),
    ("ZL=F", "ag", "Soybean Oil"),
    ("ZO=F", "ag", "Oats"),
    ("ZR=F", "ag", "Rough Rice"),
    ("HE=F", "ag", "Lean Hogs"),
    ("LE=F", "ag", "Live Cattle"),
    ("GF=F", "ag", "Feeder Cattle"),
    ("CC=F", "ag", "Cocoa (ICE US)"),
    ("KC=F", "ag", "Coffee (ICE US)"),
    ("CT=F", "ag", "Cotton (ICE US)"),
    ("SB=F", "ag", "Sugar No. 11 (ICE US)"),
    ("OJ=F", "ag", "Orange Juice (ICE US)"),
    # fx
    ("6E=F", "fx", "Euro FX"),
    ("6J=F", "fx", "Japanese Yen"),
    ("6B=F", "fx", "British Pound"),
    ("6C=F", "fx", "Canadian Dollar"),
    ("6A=F", "fx", "Australian Dollar"),
    ("6S=F", "fx", "Swiss Franc"),
    ("6N=F", "fx", "New Zealand Dollar"),
    ("6M=F", "fx", "Mexican Peso"),
    ("DX=F", "fx", "US Dollar Index (ICE US)"),
    # crypto
    ("BTC=F", "crypto", "Bitcoin (CME)"),
    ("ETH=F", "crypto", "Ether (CME)"),
]


def _universe_us_futures(spec: ExchangeSpec, args) -> pd.DataFrame:
    return pd.DataFrame(
        [{"ticker": t, "code": t, "sector": g, "name": n} for t, g, n in _US_FUTURES]
    )
...
EXCHANGES: dict[str, ExchangeSpec] = {
    ...
    # Not a venue: the CBOE US Treasury yield indices (^IRX/^FVX/^TNX/^TYX).
    # Routed to its own table rateIndices_m1_yfinance; `exchange column = rateidx.
    # tz/session are the US cash bond hours, for live-feed gating only.
    "rateidx": ExchangeSpec(
        key="rateidx", name="CBOE US Treasury yield indices", tz="America/New_York",
        poll_start=(8, 0), poll_end=(17, 15), yahoo_suffix="",
        _to_yahoo=_yahoo_suffix_only, _universe=_universe_rate_indices,
        table="rateIndices_m1_yfinance",
    ),
    # Not a venue: curated CME Group / ICE US continuous front-month futures
    # ("=F" tickers). Own table futures_m1_yfinance; `exchange column = futures.
    # Globex runs ~23h/day so the poll window is wide (weekday check still
    # applies - misses the Sunday-evening reopen; use --no-session for that).
    "futures": ExchangeSpec(
        key="futures", name="US futures (CME Group / ICE US front-month)", tz="America/New_York",
