...
# Same as scripts/startStop/startupAll.sh - starts every pipeline in this repo at
...
# scripts/startStop/startupAll.sh/shutdownAll.sh (same process set, generator off
...
#
# Usage: ./scripts/startStop/startupAllWithGen.sh
...
if ! command -v "$Q" > /dev/null 2>&1; then
  ...
  echo "Set Q_BIN to your q executable, e.g. Q_BIN=/path/to/q ./scripts/startStop/startupAllWithGen.sh" >&2
  exit 1
fi
...
if [ -f "$PIDFILE" ] && kill -0 $(cut -d' ' -f2 "$PIDFILE" | head -1) 2>/dev/null; then
  ...
  echo "Stop it first with ./scripts/startStop/shutdownAllWithGen.sh before starting again." >&2
  exit 1
fi
...
echo "To stop everything: ./scripts/startStop/shutdownAllWithGen.sh"
