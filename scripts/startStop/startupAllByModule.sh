#!/bin/bash
# scripts/startStop/startupAllByModule.sh
# Starts every process one module defines under cfg_proc/modules/<name>/ -
# e.g. ./scripts/startStop/startupAllByModule.sh mon starts mon's
...
# (cfg_proc/*.json directly) is scripts/startStop/startup.sh's job, not this one's.
#
# Usage: ./scripts/startStop/startupAllByModule.sh <module-name>
#   e.g. ./scripts/startStop/startupAllByModule.sh mon
...
# MODID: MODULE's own basename - identical to MODULE for every plain
# (non-nested) module name, but for a nested one like yfinance/eq_m1_yfinance
# (cfg_proc/modules/yfinance/<name>/ - see cfg_proc/modules/README.md) this
# is what actually names the PID file, log files, and examples/data dir, so
# they stay exactly eq_m1_yfinance-shaped regardless of where its config
# lives under cfg_proc/modules/ - unaffected by directory reorgs.
MODID="$(basename "$MODULE")"
DATA="$ROOT/examples/data/$MODID"
Q="${Q_BIN:-q}"
...
if [ ! -d "$MODCFG" ]; then
  ...
  ls -1 "$ROOT/cfg_proc/modules" | grep -v -E '^(yfinance|README\.md)$' >&2
  echo "  (yfinance/* - one of:)" >&2
  ls -1 "$ROOT/cfg_proc/modules/yfinance" | sed 's/^/    yfinance\//' >&2
  exit 1
fi
...
if [ -f "$PIDFILE" ] && kill -0 $(cut -d' ' -f2 "$PIDFILE" | head -1) 2>/dev/null; then
  ...
  echo "Stop it first with ./scripts/startStop/shutdownAllByModule.sh $MODULE before starting again." >&2
  exit 1
fi
...
start_proc() {
  ...
  echo "=== starting ${MODID}_${role} ($config) ==="
  (cd "$CORE" && exec "$Q" initFromCfg.q -config "$config" "$@" > "$LOGS/bymod_${MODID}_${role}.log" 2>&1) &
  echo "${MODID}_${role} $!" >> "$PIDFILE"
  sleep 1
}
...
echo "Logs in: $LOGS/bymod_${MODID}_<role>.log"
echo "To stop: ./scripts/startStop/shutdownAllByModule.sh $MODULE"
