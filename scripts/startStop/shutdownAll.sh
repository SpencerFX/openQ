#!/bin/bash
# scripts/startStop/shutdownAll.sh
# Stops everything scripts/startStop/startupAll.sh started, using the PIDs it
# recorded in openq-all.pids. Safe to run even if some/all processes
# already died. Separate from scripts/startStop/shutdown.sh, which only manages the
# plain default pipeline's own openq.pids - use this one for anything
# started via startupAll.sh.
#
# Usage: ./scripts/startStop/shutdownAll.sh
set -uo pipefail
