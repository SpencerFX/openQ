...
# it as a library. For that, use scripts/startupAllWithGen.sh instead,
...
if ! command -v "$Q" > /dev/null 2>&1; then
  ...
  echo "Set Q_BIN to your q executable, e.g. Q_BIN=/path/to/q ./scripts/startupAll.sh" >&2
  exit 1
fi
...
if [ -f "$PIDFILE" ] && kill -0 $(cut -d' ' -f2 "$PIDFILE" | head -1) 2>/dev/null; then
  ...
  echo "Stop it first with ./scripts/shutdownAll.sh before starting again." >&2
  exit 1
fi
...
echo "To stop everything: ./scripts/shutdownAll.sh"
